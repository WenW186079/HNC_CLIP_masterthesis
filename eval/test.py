...
def load_finetuned_clip_model(model_name, checkpoint_path, device='cuda'):
...
def print_comparison_table(results):
    headers = ["Checkpoint", "Eval Method", "Avg_Pos", "Avg_Neg", "Margin"]
    extra = "Avg_Rand_Neg"
    if any(extra in r for r in results):
...
def main():
    ...
    # Change checkpoint_path to accept one or more checkpoints (nargs='+')
    ...
    parser.add_argument("--eval_method", type=str, default="random", choices=["cosine", "plot", "random"],
    ...
    results = []  # To collect evaluation results
    
    # If model_type is "finetuned", iterate over each checkpoint path.
    ...
    for cp_path in checkpoint_paths:
        if args.model_type == "finetuned" and cp_path is not None:
            model, preprocess, tokenizer = load_finetuned_clip_model(args.model_name, cp_path, device=device)
        else:
            model, preprocess = clip.load(args.model_name, device=device)
            tokenizer = clip.tokenize

        model.eval()
        test_loader, test_dataset = load_split(args.test_json, "test", args.images_path, tokenizer, preprocess, args.batch_size, subset_size=None)
        ...
        if eval_method == "cosine":
            ...
            # Use os.path.basename to get just the filename from the checkpoint path.
