...
from loss_func import CLIPLoss, StandardCLIPLoss, CombinedCLIPDPOLoss, preprocess_text_and_images_hnc, preprocess_text_and_images_standard
from val import evaluate_cosine_similarities, load_image
...
def train_clip_model(
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            pixel_values, text_inputs = preprocess_fn(batch, preprocess, device)
            ...
            if val_dataset is not None and (global_step % val_step_frequency == 0):
                ...
                avg_pos, avg_neg, margin = evaluate_cosine_similarities(model_to_eval, val_subset, preprocess, device)
                if dist.get_rank() == 0:
