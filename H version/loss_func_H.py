...
class CLIPLoss(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale, original_state_dict=None, model=None):
        ...
        weights = torch.ones(2 * B, device=logits.device)
        ...
        # Compute the text-to-image loss only on the positive captions (first B columns).
        loss_t2i = F.cross_entropy(logit_scale * logits_raw[:, :B].t(), labels)
